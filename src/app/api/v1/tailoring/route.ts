import { NextResponse } from 'next/server';
import jwt from 'jsonwebtoken';
import prisma from '@/lib/prisma';
import { generateTailoredResume, calculateAtsScore } from '@/lib/tailoring/tailoringEngine';

export async function POST(request: Request) {
  try {
    const authHeader = request.headers.get('authorization');
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return NextResponse.json({ detail: 'Not authenticated' }, { status: 401 });
    }

    const token = authHeader.split(' ')[1];
    const secret = process.env.JWT_SECRET_KEY || 'your-secret-key-change-this-in-production';
    
    let decoded;
    try {
      decoded = jwt.verify(token, secret) as { sub: string };
    } catch (err) {
      return NextResponse.json({ detail: 'Invalid token' }, { status: 401 });
    }

    const userId = parseInt(decoded.sub, 10);
    const { resumeId, jobId } = await request.json();

    if (!resumeId || !jobId) {
      return NextResponse.json({ detail: 'Resume ID and Job ID are required' }, { status: 400 });
    }

    const resume = await prisma.resume.findUnique({ where: { id: parseInt(resumeId, 10) } });
    const job = await prisma.job.findUnique({ where: { id: parseInt(jobId, 10) } });

    if (!resume || !job) {
      return NextResponse.json({ detail: 'Resume or Job not found' }, { status: 404 });
    }

    // Mock extraction if parsedContent is null
    const originalContent = resume.parsedContent || "Candidate Resume\nSkills: TypeScript, Next.js";
    const jobDescription = job.description || "Looking for a full-stack developer with Next.js skills.";

    // Calculate original ATS Score
    const originalAtsScore = calculateAtsScore(originalContent, jobDescription);

    // Call Groq AI to rewrite
    const { tailoredResume, changes } = await generateTailoredResume(originalContent, jobDescription);

    // Calculate new ATS Score
    const newAtsScore = calculateAtsScore(tailoredResume, jobDescription);

    // Save session
    const session = await prisma.tailoringSession.create({
      data: {
        userId,
        resumeId: resume.id,
        jobId: job.id,
        originalResume: originalContent,
        tailoredResume,
        jobDescription,
        originalAtsScore,
        newAtsScore,
        scoreImprovement: newAtsScore - originalAtsScore,
        status: 'completed',
        changes: JSON.stringify(changes)
      }
    });

    // Save change logs
    if (changes && changes.length > 0) {
      const logs = changes.map(change => ({
        sessionId: session.id,
        changeType: 'REWRITE',
        originalText: change.originalText,
        tailoredText: change.tailoredText,
        explanation: change.explanation
      }));
      await prisma.tailoringChangeLog.createMany({ data: logs });
    }

    return NextResponse.json(session, { status: 201 });
  } catch (error) {
    console.error('Tailoring POST error:', error);
    return NextResponse.json({ detail: 'Internal server error' }, { status: 500 });
  }
}
