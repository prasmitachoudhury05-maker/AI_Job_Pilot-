...
function formatRanking(ranking: any, job: any, candidate: any) {
...
export async function POST(request: Request) {
  try {
    ...
    const job = await prisma.job.findUnique({ where: { id: parseInt(job_id, 10) } });
    if (!job) {
      return NextResponse.json({ detail: 'Job not found' }, { status: 404 });
    }

    // Parse skills (assuming they are JSON strings)
    const candidateSkills = candidate.skills ? JSON.parse(candidate.skills) : [];
    const jobSkills = job.skills ? JSON.parse(job.skills) : [];
    ...
    const { explanation, confidence } = await getJobRankingExplanation(candidateProfileData, job, overallScore);

    // Save to Database
    const ranking = await prisma.jobRanking.create({
      data: {
    ...
    return NextResponse.json(formatRanking(ranking, job, candidate), { status: 200 });
